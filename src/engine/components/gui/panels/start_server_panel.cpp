...
void MainMenuView::stopLocalServer(std::size_t index) {
...
void MainMenuView::stopAllLocalServers() {
    for (std::size_t i = 0; i < localServers.size(); ++i) {
        stopLocalServer(i);
    }
}

std::string MainMenuView::findServerBinary() {
...
bool MainMenuView::isPortInUse(uint16_t port, int ignoreId) const {
    if (port == 0) {
        return true;
    }
...
bool MainMenuView::launchLocalServer(LocalServerProcess &server, std::string &error) {
    error.clear();
...
void MainMenuView::drawStartServerPanel(const MessageColors &colors) {
