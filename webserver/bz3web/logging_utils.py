...
def _handler_from_settings(settings, log_path):
    ...
    raise ValueError("[bz3web] Error: logging.rotation must be 'daily' or 'size'.")
...
def _get_access_logger(settings, log_path, level):
    ...
    logger = logging.getLogger(f"bz3web.access.{log_path}")
...
def init_logging(settings, community_dir):
    ...
    server_logger = logging.getLogger("bz3web")
    ...
    if not isinstance(paths, (list, dict)):
        raise ValueError("[bz3web] Error: logging.paths must be a list or map.")
    rules = []
    if isinstance(paths, list):
        entries = []
        for index, entry in enumerate(paths):
            if not isinstance(entry, dict):
                raise ValueError("[bz3web] Error: logging.paths entries must be objects.")
    ...
    for index, path_value, entry in entries:
        ...
        if not isinstance(path_value, str) or not path_value.startswith("/"):
            raise ValueError("[bz3web] Error: logging.paths entry path must be a string starting with '/'.")
        file_value = entry.get("file")
        if file_value is not None:
            if not isinstance(file_value, str) or not file_value:
                raise ValueError("[bz3web] Error: logging.paths entry file must be a non-empty string or null.")
