...
void MainMenuView::drawThemesPanel(const MessageColors &colors) {
    ensureThemesLoaded();
...
void MainMenuView::applyThemeToView(const ThemeConfig &theme) {
...
bool MainMenuView::loadUserConfig(nlohmann::json &out) const {
...
bool MainMenuView::saveUserConfig(const nlohmann::json &userConfig, std::string &error) const {
    const std::filesystem::path path = userConfigPath.empty()
        ? bz::data::EnsureUserConfigFile("config.json")
        : std::filesystem::path(userConfigPath);
    std::error_code ec;
...
void MainMenuView::setNestedConfig(nlohmann::json &root,
...
void MainMenuView::eraseNestedConfig(nlohmann::json &root,
...
nlohmann::json MainMenuView::themeToJson(const ThemeConfig &theme) const {
...
MainMenuView::ThemeConfig MainMenuView::themeFromJson(const nlohmann::json &themeJson,
...
void MainMenuView::applyThemeSelection(const std::string &name) {
    if (name == "Default") {
        resetToDefaultTheme();
        return;
    }
...
void MainMenuView::resetToDefaultTheme() {
    nlohmann::json userConfig;
    if (!loadUserConfig(userConfig)) {
        themeStatusText = "Failed to load user config.";
        themeStatusIsError = true;
...
void MainMenuView::ensureThemesLoaded() {
    if (themesLoaded) {
        return;
    }
    themesLoaded = true;
