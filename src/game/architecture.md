...
## Runtime flow (client)
1) Client initializes Karma subsystems (render, input, physics, audio, UI).
2) Connects to server (network protocol in `net/`).
3) `ClientWorldSession` loads world data and spawns game entities.
4) `Game` updates gameplay state and drives rendering.
...
- **Renderer**: BZ3’s `Renderer` uses engine graphics backends and render passes.
