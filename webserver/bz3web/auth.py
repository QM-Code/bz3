...
def get_user_from_request(request):
    ...
    if not token:
        return None
    payload = webhttp.verify_session(token, config.require_setting(config.get_config(), "session_secret"))
    if not payload or not payload.isdigit():
        if payload and payload.startswith("admin:"):
            username = payload.split("admin:", 1)[1] or config.require_setting(config.get_config(), "admin_user")
            return {
                "id": None,
                "username": username,
                "email": "",
...
def ensure_admin_user(settings, conn):
    admin_user = config.require_setting(settings, "admin_user")
...
def display_username(user):
    if not user:
        return None
    admin_user = config.require_setting(config.get_config(), "admin_user")
    name = None
