...
from bz3web import logging_utils
...
def application(environ, start_response):
    ...
    try:
        ...
        logging.getLogger("bz3web").exception(
    ...
    try:
        ...
        if getattr(exc, "errno", None) in (98, 10048) or "Address already in use" in str(exc):
            ...
            if config.get_port_override() is not None:
                print(f"[bz3web] Error: Port {port} is already in use.")
                return
            print(
                f"[bz3web] Error: Port {port} is already in use.\n"
                f"[bz3web] Change the port in {community_dir}/config.json or use the -p <port> flag."
            )
            return
        raise
